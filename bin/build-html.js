...
for (const nextItem of await (
  await fetch("https://kherrick.github.io/slashdot/index.json")
).json()) {
...
archivesHeaderLink.setAttribute(
  "href",
  "https://github.com/kherrick/slashdot/blob/main/archives/index.md"
);
...
(
  await (
    await fetch(
      "https://raw.githubusercontent.com/kherrick/slashdot/main/archives/index.md"
    )
  ).text()
)
  .split("\n")
  ...
  .forEach((line) => {
    ...
    archiveYearLink.setAttribute(
      "href",
      "https://github.com/kherrick/slashdot/blob/main/archives/" + indexPath
    );
